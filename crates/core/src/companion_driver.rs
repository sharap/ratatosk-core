...
impl<R: Runner> CompanionDriver<R> {
    ...
    async fn on_transport(&mut self, event: TransportEvent) {
        ...
        match event {
            ...
            // ни почты.
            //
            // `Lost` — «ступень перестала работать» — сюда пока тоже
            // попадает, и это **осознанная недоделка**, а не забывчивость.
            // У терминала своя лестница (`via`), и отвалившаяся ступень
            // означала бы для неё то же, что `on_link_gone(Some(via))`.
            // Не сделано потому, что производитель этого события сегодня
            // один — меш телефона, — и подключать вторую сторону раньше,
            // чем первая проверена на устройстве, значит платить двумя
            // догадками за один прогон.
            _ => {}
        }
    }
