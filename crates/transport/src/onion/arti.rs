...
impl Runner for OnionRunner {
    async fn execute(&mut self, command: TransportCommand) -> Result<(), TransportError> {
        match command {
            ...
            // Сеть сменилась — цепочки прежней сети мертвы.
            //
            // Новые построит arti сам, и трогать клиента отсюда незачем.
            // А вот `DataStream` поверх старой цепочки лечению не подлежит,
            // и узнали бы мы о нём только первой записью — то есть кадром,
            // потраченным впустую, и сроком ожидания сверху. У onion этот
            // срок сорок пять секунд по §5.4, так что забыть связи здесь
            // дешевле всего, что можно придумать.
            TransportCommand::NetworkChanged => {
                let peers: Vec<[u8; 32]> = self.links.keys().copied().collect();
                self.links.clear();
                for peer_ik in peers {
                    // §5.4 узнаёт об обрыве сразу, а не по сроку: набирать
                    // заново он будет по тому же адресу — onion от смены
                    // сети не зависит, — но **новую** цепочку.
                    let _ = self
                        .events_tx
                        .send(TransportEvent::Disconnected { peer_ik, via: Transport::Onion })
                        .await;
                }
                Ok(())
            }
...
fn spawn_service_status_loop(
    ...
    tokio::spawn(async move {
        ...
        while let Some(status) = stream.next().await {
            ...
            // **Переход, а не первый раз.** Прежде здесь стояло «один раз»:
            // сервис объявлялся готовым и оставался таким в глазах ядра
            // навсегда, что бы с ним дальше ни случилось. На телефоне это
            // худший из возможных случаев — Tor тяжёл и держится на живом
            // соединении, — и каждое сообщение платило сорока пятью
            // секундами набора за ступень, которой нет.
            //
            // Теперь состояние сервиса и готовность ступени ходят вместе:
            // перестал быть `Running` — §5.4 его больше не выбирает, стал
            // снова — вернулся вместе с адресом. Повторное объявление того
            // же адреса ядро отбросит само (§4.3).
            //
            // Всё, что не `Running`, считается «не работает», и это
            // намеренно грубо: состояния arti здесь не разбираются
            // по одному (см. оговорку про `СВЕРИТЬ` в заголовке файла),
            // а цена ошибки в эту сторону — несколько сообщений,
            // ушедших следующей ступенью.
            if running != announced {
                announced = running;
                let event = if running {
                    TransportEvent::TorReady { onion: address.clone() }
                } else {
                    TransportEvent::Lost { transport: Transport::Onion }
                };
