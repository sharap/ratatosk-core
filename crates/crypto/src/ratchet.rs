...
impl SendChain {
    ...
    /// обратно.
    // `Iterator` здесь не реализуется намеренно, а не по недосмотру.
    // Итератор обещает, что по нему можно пройти лениво, склеить с другим,
    // вызвать `take`, `peekable` или `collect` — а цепочка ключей ни одного
    // из этих обещаний не держит: каждый вызов **необратимо** сжигает ключ
    // цепочки, и лишний шаг адаптера стоил бы рассинхрона с собеседником.
...
impl SenderInbox {
    ...
    /// Самая ранняя позиция, которую мы **ещё можем открыть** (§7.3).
    ///
    /// Это либо самый старый ключ в кэше пропусков, либо, если кэш пуст,
    /// номер, которого цепочка ждёт следующим: всё, что раньше, уже
    /// израсходовано или не выдавалось нам никогда (§11.5).
    ///
    /// # Зачем это наружу
    ///
    /// Анти-энтропия (§7.2) спрашивает у соседей пропущенное. Спрашивать
    /// то, чего мы не откроем и получив, — не бережливость, а честность
    /// перед человеком: блоки приехали бы, потратили полосу и исчезли,
    /// а история у него так и не появилась бы. Ниже этой границы
    /// спрашивать нечего, и граница эта — свойство цепочки, а не архива.
    #[must_use]
    pub fn oldest_openable(&self) -> u64 {
        self.skipped.keys().next().copied().unwrap_or(self.next_counter)
    }
...
mod tests {
    ...
    #[test]
    fn the_openable_edge_is_the_oldest_key_we_still_hold() {
        // §7.3 велит спрашивать пропущенное, а §11.5 — не выдавать
        // новичку прошлого. Граница между «спросить стоит» и «не откроем
        // и получив» одна: самый старый ключ, который у нас ещё есть.
        //
        // Числа свои, не из крейта: проверка стережёт обещание, а не
        // сегодняшний предел кэша.
        let mut send = SenderChain::new(root());
        let first_five: Vec<(u64, Key32)> = (0..5).map(|_| send.next()).collect();
        // Цепочку отдали на пятой позиции — раньше неё не открыть ничего.
        let mut handed = SenderInbox::resume(Key32::new(*send.export()), send.counter());
        let next_five: Vec<(u64, Key32)> = (0..5).map(|_| send.next()).collect();

        assert_eq!(handed.oldest_openable(), 5, "пустой кэш — граница там, где ждём следующий");
        assert!(handed.peek(first_five[0].0).is_err(), "сказанного до выдачи не открыть");

        // Принят девятый: ключи 5..8 достроились в кэш, и граница
        // остаётся на пятёрке — пропущенное открыть ещё можно.
        let (ninth, _) = next_five[4];
        handed.peek(ninth).unwrap();
        handed.commit(ninth, 0).unwrap();
        assert_eq!(handed.next_counter(), 10);
        assert_eq!(handed.oldest_openable(), 5, "пропущенное после выдачи ещё открывается");

        // А тот, кто слушал с нуля, может открыть всё с нуля.
        let mut from_zero = SenderInbox::resume(root(), 0);
        from_zero.peek(ninth).unwrap();
        from_zero.commit(ninth, 0).unwrap();
        assert_eq!(from_zero.oldest_openable(), 0);
    }
