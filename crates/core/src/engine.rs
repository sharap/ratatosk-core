...
pub enum EngineError {
    ...
    /// Ключ чтения поворачивает только владелец (фаза 2, §6.4, §3.2).
    ///
    /// §6.4 отдаёт поворот и держателю права «исключать», но §3.2 делает
    /// это невозможным по построению: новое поколение уезжает по составу,
    /// а состав у делегата — он сам. Поворот делегатом уходил в никуда:
    /// поколение росло у него одного, и его же слова с этого мига
    /// не открывал никто. Право «исключать» у делегата остаётся правом
    /// выдать ключ чтения при впуске.
    #[error("ключ чтения поворачивает только владелец: состав канала знает он один")]
    OnlyOwnerRotates,
...
/// Чем кончилось применение действия в группе.
///
/// Два исхода, а не `Option`: «применилось без новостей» и «цели ещё
/// нет» — разные вещи, и второму в канале полагается ожидание
/// (`on_group_action`), а не тишина.
enum ActionOutcome {
    /// Применилось; внутри — новости для клиента.
    Applied(Vec<Effect>),
    /// Действие ссылается на сообщение, которого у нас ещё нет.
    TargetUnknown,
}
...
pub struct ChannelFacts {
    ...
    ///
    /// `None` — ждать нечего: канал открыт, либо это не канал вовсе.
    /// Отсчёт идёт от **первой** просьбы (§10.5 меряет ожидание от неё),
    /// и повтор его не двигает.
    ///
...
pub struct Engine<S: Store> {
    ...
    ///
    /// Значение — когда привязались в последний раз, мс. Привязка живёт
    /// столько, сколько связь (§7.5.1), а связь на нашей стороне видна
    /// только повторной привязкой: читатель называется снова на каждом
    /// обходе и на каждой новой сессии. Не назвался дольше
    /// [`swarm::ATTACH_TTL_MS`] — забывается (`keep_catalogue_fresh`).
    /// Отказы ступеней для этого не годятся: стенд теряет кадры штатно,
    /// а Tor-узел с одной ступенью исчерпывает лестницу одним отказом.
    attached: BTreeMap<ChatId, BTreeMap<[u8; 32], u64>>,
    ...
    /// Каналы, от которых отписались (§10.6), — чтобы их блоки, которые
    /// владелец и сиды слать не перестанут, не ложились в очередь
    /// отложенного как «кадр про группу, в которую нас ещё не позвали».
    ///
    /// На диске (`META_CHANNELS_LEFT`), потому что слать их будут
    /// и после перезапуска. Подписка заново снимает канал из списка.
    left_channels: Vec<ChatId>,
    /// Сколько роевых кадров какого вида мы собрали (§7.1) — **ради
    /// разбора**.
    ///
    /// # Зачем это ядру
    ///
    /// Все пять кадров дерева едут одним видом (`SwarmControl`), и счёт
    /// по виду их не различает — а различать приходится: §8.4 запрещает
    /// эфиру `Want` и не запрещает `IHave`. Снаружи «попросили и молчат»
    /// и «не попросили вовсе» выглядят одинаково, и отличить их иначе
    /// нечем.
    ///
    /// Считается **сборка**, а не отправка: правило §8.4 спрашивается
    /// до неё, и проверять надо именно то, что кадр не собрался.
    ///
    /// В памяти: это не состояние мира, а след прогона. Границу UniFFI
    /// не пересекает (§13.3) — клиенту показывать нечего.
    swarm_controls: BTreeMap<&'static str, u64>,
    ...
    /// Кто чем держит этот файл — карта кусков на держателя (§9.1).
    ///
    /// **В памяти, и по той же причине, что привязка** (§7.5.1): это
    /// знание о живых связях, а не о мире. Держатель, объявившийся
    /// полгода назад, сегодня недостижим, и хранить его на диске значило
    /// бы ходить к нему при каждом запуске.
    file_holders: BTreeMap<FileId, BTreeMap<[u8; 32], Vec<u8>>>,
    /// У кого последний раз просили куски этого файла — чтобы вычеркнуть
    /// замолчавшего из держателей (`on_file_stall`).
    file_asked: BTreeMap<FileId, [u8; 32]>,
...
impl<S: Store> Engine<S> {
    ...
    pub fn new(
        ...
        Engine {
            ...
            file_asked: BTreeMap::new(),
            history_pull: BTreeSet::new(),
            tree: BTreeMap::new(),
            swarm_controls: BTreeMap::new(),
    ...
    pub fn restore(&mut self) -> Result<usize, EngineError> {
        ...
        // Отписанные каналы — до отложенного: отложенное про них
        // разбирать незачем.
        if let Some(raw) = self.store.meta(ratatosk_store::META_CHANNELS_LEFT)? {
            self.left_channels =
                raw.chunks_exact(16).filter_map(|chunk| <[u8; 16]>::try_from(chunk).ok()).collect();
        }
        for parked in self.store.pending_group()? {
            let Ok(envelope) = Envelope::decode(&parked.envelope) else { continue };
            if self.left_channels.contains(&parked.chat_id) {
                continue;
            }
