...
/// Метка операции состава целиком: чат, время HLC (стена и логика),
/// автор, номер и цель. Шестёрка вынесена в имя не ради краткости —
/// она обязана совпадать с первичным ключом `group_members` в файловой
/// базе поле в поле, и два места, где её надо держать одинаковой,
/// лучше назвать одним словом.
type MembershipKey = ([u8; 16], u64, u32, [u8; 32], [u8; 8], [u8; 32]);
...
pub struct MemoryStore {
    ...
    /// Порядок обхода задан ключом и совпадает с `ORDER BY` файловой базы.
    membership: BTreeMap<MembershipKey, bool>,
