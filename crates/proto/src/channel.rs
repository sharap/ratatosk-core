...
mod tests {
    ...
    #[test]
    fn the_private_channel_text_does_not_promise_a_request_we_do_not_send() {
        // **Расхождение со спекой, записанное проверкой.** §15 обещает
        // «Он увидит вашу карточку», а заявки (§10.4) в ядре нет:
        // подписка по ссылке не отправляет владельцу ничего. Текст,
        // обещающий это сегодня, был бы прямой ложью — человек ждал бы
        // ответа на то, чего не отправлял.
        //
        // Проверка падает, когда заявку напишут: тогда константу надо
        // поменять и вернуть фразу спеки.
        assert!(!PrivateChannelConsequences::OWNER_LEARNS_BY_ITSELF);
        let text = PrivateChannelConsequences::ui_text();
        assert!(!text.contains("карточку"), "не обещаем того, чего не отправляем");
        assert!(text.contains("другим способом"), "и говорим, что делать вместо");
    }
                    ...
                    self.endpoints
                        ...
                        .map(|endpoint| {
                            let value = match endpoint {
                                ...
                                Endpoint::Ygg(key) => Value::Bytes(key.to_vec()),
                            };
                            Value::Map(vec![
                                (
...
///
/// Показывается **до** подписки и до того, как ссылкой поделятся.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl PrivateChannelConsequences {
    ...
    ///
    /// **Нет, и это расхождение со спекой.** §15 обещает «он увидит вашу
    /// карточку», а заявки (§10.4) в ядре нет: подписка по ссылке
    /// не отправляет владельцу ничего. Пока это так, текст не вправе
    /// обещать, что владелец о вас узнает, — и не обещает.
    pub const OWNER_LEARNS_BY_ITSELF: bool = false;

    /// Точная формулировка для UI (§15, поправленная).
    ///
    /// От спеки отличается последней фразой: «Он увидит вашу карточку»
    /// заменено на то, что верно сегодня. Вернуть её можно будет вместе
    /// с заявкой.
    #[must_use]
    pub const fn ui_text() -> &'static str {
        "Впустить вас должен владелец канала или тот, кому он это доверил. \
         Пока вас не впустят, канал не откроется. Сказать ему о себе через \
         саму ссылку нельзя — попросите его другим способом."
    }
}
