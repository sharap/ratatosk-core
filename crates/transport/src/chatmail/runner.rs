...
enum Job {
    ...
    ///
    /// Почте это нужно не меньше прямых ступеней, и на приёме — больше.
    /// SMTP-соединение прежней сети мертво, но узнает об этом только
    /// первое письмо; а `IDLE` по IMAP ждёт **часами** и на мёртвом сокете
    /// ждать будет ровно столько же. То есть без этой работы входящая
    /// почта после переезда останавливается до тех пор, пока система
    /// не сдастся сама, — и человек видит не «почта не работает», а тишину.
    NetworkChanged,
    /// Отправить письмо.
    Letter {
...
async fn serve(
    ...
    loop {
        ...
        match job {
            Job::NetworkChanged => {
                // Отправку роняем, а не переоткрываем здесь же: письма
                // может и не быть, а вход на сервер — это секунды напрямую
                // и десятки через Tor. Откроет его первое же письмо
                // (`send_letter` входит заново, если соединения нет),
                // и заплатит за это только оно.
                sender = None;
                // А приём поднимаем **сразу**, и в этом вся разница.
                // Ему никто не «первое письмо»: задача сидит в `IDLE`
                // на мёртвом сокете и будет сидеть часами. Уронить
                // и поднять — единственный способ это прекратить.
...
impl Runner for MailRunner {
    async fn execute(&mut self, command: TransportCommand) -> Result<(), TransportError> {
        match command {
            ...
            // `try_send`, как и у соседей: ждать места в очереди значило бы
            // остановить цикл драйвера. Потерять эту работу при переполнении
            // не страшно — очередь полна ровно тогда, когда почта и так
            // занята, а занята она может быть только живым соединением.
            TransportCommand::NetworkChanged => {
                let jobs = self.jobs();
                let _ = jobs.try_send(Job::NetworkChanged);
                Ok(())
            }
