...
mod tests {
    ...
    #[test]
    fn the_waiting_screen_follows_the_clock_of_the_spec() {
        // §10.5 расписан по времени, и числа здесь повторены **сами**:
        // возьми их проверка у кода, сдвиг отметки сдвинул бы и её,
        // и она смолчала бы о том, что «дольше обычного» стало
        // появляться через час.
        assert_eq!(Waiting::at(0), Waiting::Opening);
        assert_eq!(Waiting::at(29_999), Waiting::Opening);
        assert_eq!(Waiting::at(30_000), Waiting::Longer);
        assert_eq!(Waiting::at(299_999), Waiting::Longer);
        assert_eq!(Waiting::at(300_000), Waiting::SlowPath);
        assert_eq!(Waiting::at(86_399_999), Waiting::SlowPath);
        assert_eq!(Waiting::at(86_400_000), Waiting::NoAnswer);

        // Кнопка «сообщить, когда откроется» появляется вместе
        // с медленным путём и не исчезает (§10.5).
        assert!(!Waiting::Opening.offer_a_notification());
        assert!(!Waiting::Longer.offer_a_notification());
        assert!(Waiting::SlowPath.offer_a_notification());
        assert!(Waiting::NoAnswer.offer_a_notification());
    }
    ...
    fn no_text_is_empty_or_a_single_word() {
        ...
        // Признаки §15 идут сюда же и по тому же правилу: они такие же
        // тексты, просто показываются не перед действием, а вместо
        // пустого экрана. Пустая строка у `Signal::Fine` — единственное
        // исключение, и она названа поимённо: показывать там нечего.
        let signals = [
            Signal::Awaiting,
            Signal::NotReadable,
            Signal::NobodyServes,
        ...
        .map(Signal::ui_text);
        assert!(Signal::Fine.ui_text().is_empty(), "«всё хорошо» на экране — шум");
        // Короткие надписи ожидания — по своему правилу: не пустые,
        // не обрывок, без следов склейки. Длины им не положено.
        for text in [Waiting::Opening.ui_text(), Waiting::Longer.ui_text()] {
            assert!(!text.is_empty(), "надпись ожидания не бывает пустой");
            assert!(text.ends_with('.'), "надпись обрывается на полуслове: {text}");
        ...
        for text in [
            ...
            // объяснения значит требовать объяснения там, где объяснять
            // нечего: канал открывается, это и вся новость.
            Waiting::SlowPath.ui_text(),
            Waiting::NoAnswer.ui_text(),
        ]
        .into_iter()
        .chain(signals)
        {
...
//
// §15 перечисляет одиннадцать текстов. Три из них описывают то, чего
// ядро **не делает**:
//
...
//
// `relay_policy_notice` из этого списка ушёл: уровни раздачи §12
// появились, и текст живёт вместе с ними — `swarm::SharingLevelConsequences`.
//
...
///
/// §15 требует показывать не только тексты, но и **признаки**: «никто
/// из достижимых не отдаёт этот канал», «владелец не выходил на связь»,
/// «поворот просрочен», видимую дыру при пропуске. Здесь они сведены
/// в один ответ: на экране нужен **один** повод, а держать их может
/// несколько разом.
///
/// # Почему признак один, а не список
///
/// Список означал бы, что выбор главного делает клиент, — и в каждом
/// клиенте это правило написали бы по-своему. Порядок же содержателен:
/// он расставлен по тому, **что человек может с этим сделать**, и это
/// такое же свойство протокола, как форма кадра. Сам порядок живёт
/// в ядре (`ChannelFacts::signal`), потому что складывается он
/// из фактов, которых у протокола нет.
///
/// # Чего признак не обещает
///
/// [`Signal::Fine`] — это «объяснять нечего», а не «всё доехало».
/// Пустой канал, в котором просто ничего не говорили, выглядит точно
/// так же, и отличить их ядру нечем.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Signal {
    ...
    NotReadable,
    /// Никто из достижимых не отдаёт этот канал (§7.5).
    NobodyServes,
    /// Раздающие известны, но ни один сейчас не отвечает (§7.5.1).
    ///
    /// Отдельно от [`Signal::NobodyServes`], и разница не косметическая:
    /// там раздавать некому и помочь может только новый сид, здесь дело
    /// в связи — и человеку стоит проверить её, а не искать ссылку
    /// заново.
    SeedsUnreachable,
...
impl Signal {
    /// Точная формулировка для UI (§15).
    ///
    /// Пустая строка у [`Signal::Fine`] — не забывчивость: показывать
    /// нечего, а строка «всё хорошо» на экране это шум, который человек
    /// через неделю перестанет читать вместе со всеми остальными.
    #[must_use]
    pub const fn ui_text(self) -> &'static str {
        match self {
            Signal::Fine => "",
            Signal::Awaiting => {
                "Заявка отправлена. Впустить вас должен владелец, и он должен быть \
                 для этого на связи — пока он не ответит, канал не откроется."
            }
            Signal::NotReadable => {
                "Ключа чтения у нас ещё нет: записи этого канала приедут и не \
                 откроются. Ключ везёт владелец, и он вправе опоздать."
            }
            Signal::NobodyServes => {
                "Никто из достижимых сейчас не отдаёт этот канал. Каналы держатся \
                 на том, что читатели раздают друг другу: пока раздающих нет, новые \
                 записи взять неоткуда."
            }
            Signal::SeedsUnreachable => {
                "Раздающие у этого канала есть, но связи ни с одним из них сейчас \
                 нет. Это про сеть, а не про канал: появится связь — записи \
                 подтянутся сами."
            }
            Signal::OwnerUnseen => {
                "От владельца канала давно ничего не приходило. Это не значит, что \
                 он пропал: мы видим только то, что доехало до нас."
            }
            Signal::Waiting => {
                "Записи объявлены и ещё едут. По медленным путям — почте и реле — \
                 это часы, а не секунды."
            }
            Signal::RotationOverdue => {
                "Ключу чтения больше месяца. Повернуть его стоит: все, кого нет \
                 в вашем списке читателей, потеряют доступ к новым записям, а те, \
...
///
/// # Экран переключается раньше механизма
///
/// §10.5 говорит это прямым текстом: «отметка 0:30 меняет только
/// надпись». Расписание повторов живёт своей жизнью
/// (`transport_policy::retry_after_ms`), а здесь — только то, что
/// человек видит. Свяжи мы их — надпись «дольше обычного» появлялась бы
/// не через полминуты, а когда очередь соизволит сделать шаг.
///
/// # Отсчёт идёт от **первой** просьбы
///
/// §10.5 меряет ожидание от неё, и повтор его не двигает: человек ждёт
/// с того мига, как нажал, а не с того, как ядро постучало в последний
/// раз.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Waiting {
    /// До полуминуты: «открываем канал».
    Opening,
    /// От получаса… нет, от **полуминуты** до пяти минут: «дольше
    /// обычного». Механизм при этом не меняется — меняется надпись.
    Longer,
    /// От пяти минут до суток: «медленный путь, это часы».
    ///
    /// Здесь же §10.5 велит показать кнопку уведомления: ждать часами,
    /// глядя в экран, никто не станет.
    SlowPath,
    /// Сутки прошли: «не отвечает» и повтор.
    ///
    /// Ядро к этому времени стучать перестало (`retry_after_ms` отдаёт
    /// `None`), и продолжения без человека не будет.
    NoAnswer,
}

impl Waiting {
    /// Что показывать, когда ждём столько-то.
    ///
    /// Числа повторены здесь **сами**, а не взяты у расписания повторов:
    /// §10.5 разводит экран и механизм нарочно, и связанные одной
    /// константой они разошлись бы с этим обещанием молча.
    #[must_use]
    pub const fn at(elapsed_ms: u64) -> Waiting {
        if elapsed_ms < 30_000 {
            Waiting::Opening
        } else if elapsed_ms < 5 * 60_000 {
            Waiting::Longer
        } else if elapsed_ms < 24 * 60 * 60_000 {
            Waiting::SlowPath
        } else {
            Waiting::NoAnswer
        }
    ...
    /// Нужна ли кнопка «сообщить, когда откроется» (§10.5).
    ///
    /// Появляется вместе с медленным путём и не исчезает: часы ожидания
    /// — ровно тот случай, ради которого она и заведена.
    #[must_use]
    pub const fn offer_a_notification(self) -> bool {
        matches!(self, Waiting::SlowPath | Waiting::NoAnswer)
    }

    /// Точная формулировка для UI (§15, §10.5).
    #[must_use]
    pub const fn ui_text(self) -> &'static str {
        match self {
            Waiting::Opening => "Открываем канал.",
            Waiting::Longer => "Открываем канал. Это занимает дольше обычного.",
            // Текст §15 (`channel_slow_path_notice`) — здесь и только
            // здесь: он про то же состояние, и второй его экземпляр
            ...
            Waiting::NoAnswer => {
                "Канал не отвечает сутки. Мы перестали стучать — попробуйте ещё раз, \
