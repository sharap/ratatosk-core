...
pub const fn stall_ms(via: Transport) -> u64 {
    match via {
        ...
        // два мебибайта; при скромных восьми килобайтах в секунду
        // (`floor_bytes_per_sec`) они идут больше четырёх минут. Срок
        // молчания короче этого не ускорил бы возобновление, а сломал бы
        // передачу совсем — получатель объявлял бы застоем чанк, который
        // в этот миг идёт по воздуху.
        //
        // Десять минут — те же четыре с лишним с запасом вдвое. Стояло
        // здесь пять минут, посчитанные от вдвое более быстрой скорости;
        // после замера на связке с телефоном запас съёжился почти
        // до нуля. Стоит он дёшево: обрыв канала ловится не этим сроком,
        // а разрывом связи, и срок — последняя страховка.
        Transport::Bt => 600_000,
