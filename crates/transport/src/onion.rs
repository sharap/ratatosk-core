...
// Здесь стояла `OnionService` — заглушка этапа 2 с тремя `todo!`.
//
// Этап 2 закрылся не ею: сервис поднимает `onion::arti`, держит его
// `Arc<RunningOnionService>`, а адрес приезжает событием `TorReady`.
// Заглушка пережила собственную задачу на три поставки, потому что
// на неё никто не ссылался — и потому же её не было видно.
//
// Правило отсюда то же, что записано в `HANDOFF.md` §1 про осиротевший
// `now_ms`: **убрав слой, проверь, что не осталось того, что его
// обслуживало.** `todo!` в мёртвом коде хуже, чем в живом: он числится
// незакрытой работой, которой давно нет.
