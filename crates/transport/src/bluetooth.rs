...
// Радио этой машины — только там, где есть BlueZ. Признак `bt` вне Linux
// включать можно, и он ничего не сломает: модуля просто нет, ступень
// собирается мостом. Разбор — у признака в манифесте.
...
/// MTU приёма, который мы требуем у L2CAP, — целый кадр класса S.
///
/// # Зачем его вообще выставлять
///
/// Потому что иначе его выбирает BlueZ, а его умолчание для LE CoC
/// заметно меньше нашего кадра и вдобавок зависит от ядра — то есть
/// от машины. Собеседнику при таком MTU приходится резать каждый кадр
/// на несколько пакетов, и вся дуга 0.4.8 про то, чем дробление кадра
/// кончается на приёмной стороне Android.
///
/// Числа умолчания здесь нарочно не названо: гадать о чужом ядре нечего,
/// а получившийся MTU радио называет в журнале само — и просимый,
/// и выданный.
///
/// # Почему ровно кадр класса S, а не «побольше»
///
/// Ради **совместимости с телефоном**, и она здесь важнее скорости.
/// На Android приёмный пакет равен 8087 байт и берётся у системы,
/// а не выбирается нами; кадр класса S в него влезает целиком, и один
/// кадр там всегда один пакет. Прибив то же число здесь, мы получаем
/// одинаковый провод в обе стороны: ПК↔телефон ведёт себя так же,
/// как телефон↔телефон.
///
/// Цена названа прямо: между двумя Linux-машинами BlueZ мог бы
/// договориться на пакет покрупнее и возить быстрее. Пока эфир
/// не выровнен по обеим платформам, эта скорость нам не нужна —
/// она нужна только в паре, которой у людей не бывает.
///
/// Плюс байт класса: кадр едет вместе с ним (`link::write_frame`),
/// и не влезь этот байт — резать начнут ровно на нём.
///
/// # Почему константа живёт здесь, а не в `local.rs`
///
/// Потому что это свойство **ступени**, а не радио Linux: наибольший
/// класс, который эфир возит, решает §9.3, и MTU обязан ехать за ним.
/// Связь двух чисел держит проверка
/// (`the_air_mtu_holds_a_whole_frame_of_the_class_the_rung_carries`),
/// и стоять она должна там, где компилируется **всегда** — иначе тот, кто
/// поднимет потолок класса, не собирая признак `bt`, не узнает об этом
/// вовсе.
///
/// Отсюда и разрешение ниже: без признака `bt` единственный настоящий
/// потребитель константы не собирается, и обычная сборка честно говорит
/// «не используется». Заглушить это молча было бы неправдой, а убрать
/// константу — потерять проверку; поэтому разрешение **узкое**, ровно
/// на тот случай, когда десктопного радио в сборке нет.
#[cfg_attr(
    not(all(feature = "bt", target_os = "linux")),
    allow(dead_code, reason = "потребитель — радио Linux")
)]
pub(crate) const AIR_RECV_MTU: u16 = SizeClass::S.frame_len() as u16 + 1;
...
mod tests {
    ...
    #[test]
    fn the_air_mtu_holds_a_whole_frame_of_the_class_the_rung_carries() {
        // **Два числа из разных крейтов, и разойтись им нельзя.** MTU
        // приёма мы просим у L2CAP сами (`local.rs`), а наибольший класс,
        // который ступень возит, решает §9.3 (`fragment::ceiling`). Подними
        // кто-нибудь потолок до класса M — и кадр перестанет влезать
        // в пакет, а узнаем мы об этом не здесь, а неделей разбора
        // на стенде: дробление кадра ровно так и ломало приём Android
        // дважды за дугу 0.4.8.
        //
        // Проверка потому и стоит в транспорте, а не рядом с константой:
        // видеть надо оба числа сразу.
        let ceiling = ratatosk_proto::fragment::ceiling(Transport::Bt);
        assert_eq!(
            ceiling,
            SizeClass::S,
            "ступень возит класс {ceiling:?} — MTU приёма обязан поехать за ним"
        );
        // Плюс байт класса: кадр едет вместе с ним (`link::write_frame`).
        let whole = ceiling.frame_len() + 1;
        assert!(
            usize::from(AIR_RECV_MTU) >= whole,
            "кадр занимает {whole} байт, а просим мы {AIR_RECV_MTU}: собеседник \
             будет резать каждый кадр на пакеты"
        );
        // И не «побольше на всякий случай»: число обязано быть тем же,
        // что у телефона, иначе провод в две стороны разный.
        assert!(
            usize::from(AIR_RECV_MTU) <= whole + 64,
            "MTU оторвался от кадра: просим {AIR_RECV_MTU} при кадре {whole}"
        );
    }
