...
//!
//! На Linux ступень работает своим радио (`local`): `bluer` даёт
...
impl BridgedAir {
    ...
    pub fn set_radio(&self, radio: Arc<dyn BtRadio>) {
        if let Ok(mut held) = self.inner.radio.lock() {
            *held = Some(Arc::clone(&radio));
        }
        // Радио могло опоздать к подъёму, и на Android оно опаздывает
        // всегда: ступень поднимается из сохранённых настроек в первые
        // же миллисекунды после открытия хранилища, а радио вручает
        // служба, которой ещё надо подняться самой. Разрыв невелик —
        // десятки миллисекунд, — но `raise` к тому моменту уже отработал
        // впустую («радио платформы не вручено»), и позвать его второй
        // раз некому: `SetEnabled` больше не придёт, а `NetworkChanged`
        // эфиру безразличен.
        //
        // Снаружи это выглядело так: Bluetooth включён, а ступень мертва
        // до ручного переключения в настройках.
        //
        // Поэтому опоздавшее радио запускаем сами — тем же хвостом, что
        // и `raise`: поднимется сокет, платформа скажет номер канала
        // в `on_ready`, и объявление соберётся.
        let raised = self.inner.setup.lock().is_ok_and(|held| held.is_some());
        if raised {
            tracing::info!("эфир: радио вручено при уже поднятой ступени — запускаем");
            radio.start();
        }
    }
