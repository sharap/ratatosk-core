...
impl<R: Runner> CompanionDriver<R> {
    ...
    async fn on_transport(&mut self, event: TransportEvent) {
        ...
        match event {
            ...
            // ни почты.
            //
            // `SeenOnBt` (0.4) сюда не приходит вовсе: раннера эфира
            // у терминала нет. Приписать его к маяку было бы неправдой
            // и в другую сторону — тот возвращает канал на `Lan`,
            // а слышимость в Bluetooth про локальную сеть не говорит
            // ничего.
            //
