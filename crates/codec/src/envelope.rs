...
pub enum PayloadType {
    ...
    GroupAction,
    /// Кусок конверта, не поместившегося в кадр (§9.3).
    ///
    /// **Несущий тип, а не содержательный.** Нагрузка такого конверта —
    /// байты **другого** конверта, порезанного на части; заголовок
    /// [`Fragment`] говорит, какая это часть и сколько их всего. Собрав
    /// все, получатель разбирает исходный конверт и дальше не отличает
    /// его от приехавшего целым.
    ///
    /// Зачем это нужно: класс кадра — потолок для содержимого, и потолок
    /// у ступеней разный. Кадр класса L до Android не доходит вовсе
    /// (0.4.8), а сообщение с десятком превью в класс M не влезает. Без
    /// фрагментации выбор был бы только между «урезать содержимое»
    /// и «не доехать».
    ///
    /// Сборка постарше увидит здесь [`PayloadType::Unknown`] и кадр
    ...
    /// вместе с нами, и отвечает на вопрос, которого у переписки
    /// не было вовсе: **у кого просить куски**. В переписке ответ один —
    /// у отправителя; в канале он не годится, потому что читатели друг
    /// друга не знают (§3.2), и файл подписчика доходил бы только
    /// до владельца.
    ///
    /// §9.1 предлагает объявлять **карту кусков**; здесь объявляется
    /// только законченное — половина, которой хватает, чтобы у файла
    /// появился второй источник.
    FileHave,
    /// «Покажи представление этого канала» (фаза 2, §10.3, шаг 2).
    ///
    /// Едет **один на один** тому, чей адрес лежит в ссылке, — владельцу
    /// или сиду. §10.3 велит достать представление **до** всякого показа
    /// содержимого, и другого пути к нему у подписчика по ссылке нет:
    /// у открытого канала состава не существует (§6.1), а значит веер
    /// владельца до него не доходит.
    ///
    /// **Это не заявка §10.4.** Заявка говорит «впустите меня» и бывает
    /// только у канала по приглашению; эта просьба говорит «покажите
    /// подписанный документ» и бывает только у открытого, где ключ
    /// чтения и так в ссылке. Смешать их нельзя: у первой ответ —
    /// впуск, у второй — документ.
    ///
    /// **Цена названа заранее** (§10.3, §15): «тот, чей адрес в ссылке,
    /// увидит, что кто-то интересуется каналом, — даже если вы потом
    /// откажетесь».
    ChannelIntroWanted,
...
impl PayloadType {
    /// Заводит ли такая нагрузка **личный разговор** с отправителем
    /// (§8.3, приёмная сторона).
    ///
    /// # Зачем это нужно
    ///
    /// Рукопожатие от незнакомца контакта больше не заводит: за сессией
    /// стоит запись пира (§8.3, «третий вид записи»), иначе список
    /// знакомых у читателя двух каналов зарастает сотнями тех, с кем
    /// человек не разговаривал. Но разговор-то начаться может — и вот
    /// тогда контакт нужен: у личного чата без него нет ни строки
    /// в списке, ни имени, ни отметки сверки.
    ///
    /// Здесь и лежит граница между «прислал кадр» и «написал мне».
    ///
    /// # Почему исчерпывающим match, а не списком
    ///
    /// Чтобы новый тип нагрузки нельзя было завести, не ответив
    /// на вопрос «а это начало разговора?». Список прошёл бы сборку
    /// молча, и новый вид кадра от незнакомца либо заводил бы контакт
    /// ни за что, либо не заводил там, где надо.
    #[must_use]
    pub const fn starts_a_personal_chat(self) -> bool {
        match self {
            // Слово, файл, реакция, правка, отзыв, пересланное, ответ,
            // чужой контакт — всё это адресовано **мне лично**, и место
            // ему в личном чате.
            PayloadType::Text
            | PayloadType::Forward
            | PayloadType::Reply
            ...
            // Лицо — тоже разговор, и это не щедрость. §4.2 отправляет
            // аватарку **только сверенному** контакту: приславший знает
            // нас лично и держит нашу карточку. А приехать она вправе
            // раньше первого слова — лестница §5.4 у кадров разной длины
            // разная, — и не заведи она контакт, лицо легло бы в пустоту
            // и нашлось бы только после следующего рукопожатия.
            | PayloadType::Avatar => true,
            // Групповое и канальное — разговор **чата**, а не со мной:
            // участник группы контактом заводится своим путём (§11.5),
            // а заявитель в канал остаётся пиром до впуска (§10.4).
            ...
            // Просьба показать представление — про канал, а не про
            // разговор: спрашивающий подписчик нам не собеседник.
            ...
            // человека менял бы кадр, пришедший неизвестно от кого.
            PayloadType::Receipt
            | PayloadType::CardUpdate
            | PayloadType::CompanionRequest
            | PayloadType::CompanionResponse
            | PayloadType::CompanionNotice => false,
            // Несущий тип: своего содержания у него нет, решает собранный
            // конверт — он придёт сюда же (`on_fragment`).
            PayloadType::Fragment => false,
            // Незнакомое не показывают и разговором не считают.
            PayloadType::Unknown(_) => false,
        }
    }
