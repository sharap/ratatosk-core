...
/// Откуда числа:
///
/// * **эфир** — измерено на стенде, и мерялось дважды. Сперва между двумя
///   машинами: кадр в 64 КиБ проходит круг с квитанцией за 370–510 мс,
///   мебибайт идёт около сорока секунд, — отсюда стояли двадцать килобайт
///   в секунду. Связка с телефоном оказалась вдвое медленнее: чанки класса
///   L шли с промежутком 84 секунды в одну сторону и 112 секунд в другую,
///   то есть 9–12 килобайт в секунду.
///
///   Разница эта не косметическая. При двадцати килобайтах срок ожидания
///   мебибайта выходил 71 секунда — **короче самой передачи**, и квитанции
///   не дожидался никто: кадр посылался заново, пока первый ещё шёл
///   по воздуху. В журнале это два одинаковых кадра класса L с промежутком
///   в 67 секунд. Восемь килобайт в секунду — ниже всего измеренного,
///   как этому числу и положено;
...
pub const fn floor_bytes_per_sec(transport: Transport) -> Option<u64> {
    match transport {
        ...
        Transport::Bt => Some(8 * 1024),
        Transport::Ygg | Transport::Onion => Some(30 * 1024),
        Transport::Nostr | Transport::Mail => None,
    }
}

/// Через сколько повторить попытку доставки, мс (§10.5).
///
/// # Зачем расписание, если есть присутствие
///
/// Потому что присутствие есть **не везде**. В локальной сети о нём
/// говорит маяк §5.1: собеседник появился — отложенное поехало.
/// А в меше и onion маяка нет вовсе, и «появился» узнать неоткуда.
/// Обе стороны тогда ждут события, которое может создать только другая,
/// — и ждут до ближайшего обхода ядра, то есть до часа.
///
/// Найдено живым прогоном на меше: вернувшийся из перезапуска читатель
/// молчал три минуты, и молчал бы час. Ни одна проверка этого
/// не показывала: в симуляции задержки мгновенные, а обрыв связи
/// смоделировать было нечем.
///
/// # Числа — из §10.5
///
/// «0:30 — продолжаем стучать; 5:00 — уходим в асинхронные; далее —
/// быстрые раз в 15 минут». Здесь те же три шага; дальше повтор идёт
/// раз в пятнадцать минут и не растёт: ожидание §10.5 меряется сутками,
/// и реже пятнадцати минут оно перестаёт быть ожиданием.
///
/// # Разброс обязателен
///
/// §10.5: «популярную ссылку получают сотни людей почти одновременно;
/// без дрожания они бьют в мёртвый адрес синхронно, а поднявшийся
/// владелец получает залп». Разброс добавляет вызывающий — у него есть
/// источник случайности, у политики его нет.
/// # У расписания есть конец
///
/// §10.5: «24:00 — прекращаем: „не отвечает“ + повтор». Поэтому
/// `None` — законный ответ: стучать сутками в мёртвый адрес значит
/// тратить батарею на обещание, которого протокол не давал. Отложенное
/// при этом не пропадает — его по-прежнему разбудит присутствие
/// собеседника или обход ядра, — но своего срока у него больше нет.
#[must_use]
pub const fn retry_after_ms(step: u32) -> Option<u64> {
    match step {
        0 => Some(30_000),
        1 => Some(5 * 60_000),
        // Дальше — раз в пятнадцать минут, пока не наберутся сутки.
        // Шагов ровно столько, сколько их влезает в сутки §10.5:
        // полминуты, пять минут и девяносто пять с лишним четвертей часа.
        2..=96 => Some(15 * 60_000),
        _ => None,
    }
}

/// Насколько разбрасывать срок повтора, промилле (§10.5: «±20%»).
pub const RETRY_JITTER_PERMILLE: u32 = 200;
...
impl Transport {
    ...
    /// Слышно ли на этой ступени соседство — то есть даёт ли адрес эфир,
    /// а не карточка.
    ///
    /// Локальная сеть и Bluetooth тем и отличаются от прочих ступеней:
    /// адреса собеседника в карточке нет, и годен он, только пока его
    /// слышно **сейчас** (§5.1, 0.4). Раз адрес живёт слышимостью — вместе
    /// с ней он обязан и протухать: отсюда [`PRESENCE_TTL_MS`].
    ///
    /// У прочих ступеней адрес в карточке, и «рядом» к ним не относится:
    /// onion-адрес не становится свежее оттого, что собеседник вошёл
    /// в комнату.
    ///
    /// Перебор здесь полный нарочно, а не `_ => false`: появится седьмая
    /// ступень — компилятор потребует сказать про неё вслух, а не отнесёт
    /// молча к «прочим».
    #[must_use]
    pub const fn hears_presence(self) -> bool {
        match self {
            Transport::Lan | Transport::Bt => true,
            Transport::Ygg | Transport::Onion | Transport::Nostr | Transport::Mail => false,
        }
    }
...
/// Сколько тишины делает «рядом» неправдой (§5.1, 0.4).
///
/// # Почему у соседства обязан быть срок
///
/// Потому что «рядом» — это знание о **сейчас**, а не память о том, что
/// когда-то слышали. Отметка ставилась объявлением и снималась только
/// выключением ступени: собеседник, ушедший из комнаты, оставался «рядом»
/// до конца сеанса. Показывать такое человеку нельзя — §14 про это прямо,
/// — а §5.4 из-за этого предлагал эфир тому, кого в эфире давно нет,
/// и платил за это полным сроком набора.
///
/// # Откуда полторы минуты
///
/// Объявления идут секундами: BLE в спокойном режиме вещает раз в четверть
/// секунды, mDNS переобъявляется чаще, чем раз в минуту. Полутора минут
/// хватает с запасом — и запас этот нужен: пока открыт канал, Android
/// уводит обзор в экономный режим (`SCAN_MODE_LOW_POWER`), и объявление
/// собеседника может не попасться заметно дольше обычного. Срок покороче
/// гасил бы значок посреди разговора.
///
/// Длиннее тоже плохо, и по-человечески: пять минут — это «вышел
/// за кофе», и значок, переживающий уход, перестаёт что-либо значить.
pub const PRESENCE_TTL_MS: u64 = 90_000;

/// Ступени, на которых соседство слышно, — в порядке лестницы §5.4.
///
/// **Список берётся из лестницы и фильтруется признаком, а не пишется
/// руками.** Руками написанные списки ступеней за один день разошлись
/// с лестницей трижды, и всякий раз молча: неполный список синтаксически
/// безупречен, и сказать о нём компилятору нечего. Здесь порядок и состав
/// приходят из [`Reachability`], а отбирает [`Transport::hears_presence`];
/// появится седьмая ступень со слышимостью — она окажется тут сама.
pub fn presence_rungs() -> impl Iterator<Item = Transport> {
    Reachability::of(PeerAvailability::default())
        .rungs
        .into_iter()
        .map(|rung| rung.transport)
        .filter(|transport| transport.hears_presence())
}
...
impl Attempt {
    ...
    /// Забывает, что эту ступень уже пробовали.
    ///
    /// # Зачем это нужно, если лестница идёт вперёд
    ///
    /// Затем, что **оборванная связь — не отказ ступени**. §5.4 ведёт
    /// лестницу по ступеням: не вышло по локальной сети — идём в меш,
    /// не вышло в меше — в onion. Но обрыв соединения не говорит
    /// о ступени ничего: `Failure::Dropped` случается от смены сети,
    /// от засыпания экрана, от перезапуска приложения на той стороне,
    /// и собеседник «обыкновенно звонит снова через секунду».
    ///
    /// Считать такую связь пройденной ступенью значит сжигать рабочий
    /// путь за чужой перезапуск. Поймано на живом меше: узел, вернувшийся
    /// из перезапуска, получал молчание — его сид упирался в мёртвое
    /// соединение, объявлял ступень пройденной, а другой у него не было.
    ///
    /// Право на это даётся **один раз на доставку** (`redial_used`):
    /// второй обрыв подряд уже похож на «собеседника там нет».
    pub fn forget(&mut self, transport: Transport) {
        self.tried.retain(|tried| *tried != transport);
        self.finished = false;
    }
...
mod tests {
    ...
    #[test]
    fn presence_is_heard_exactly_where_the_air_gives_the_address() {
        // **Проверка про то, что список не разъедется с лестницей.**
        // Соседство (`PRESENCE_TTL_MS`) гасит признак у тех ступеней,
        // где адрес даёт эфир. Выпиши этот список руками — и седьмая
        // ступень со слышимостью в него не попала бы молча: признак
        // у неё зажигался бы и не гас никогда.
        let rungs: Vec<Transport> = presence_rungs().collect();
        assert_eq!(
            rungs,
            vec![Transport::Lan, Transport::Bt],
            "адрес из эфира — у локальной сети и Bluetooth, у прочих он в карточке"
        );

        // И признак, и список обязаны говорить одно и то же: список
        // получается из признака, а признак читают поодиночке.
        for transport in ALL {
            assert_eq!(
                transport.hears_presence(),
                rungs.contains(&transport),
                "{transport:?}: признак и список разошлись"
            );
        }

        // Порядок — лестничный, а не какой придётся: по нему человеку
        // показывают, где собеседник рядом, и §5.4 читается сверху вниз.
        let ladder: Vec<Transport> = Reachability::of(PeerAvailability::default())
            .rungs
            .iter()
            .map(|r| r.transport)
            .collect();
        let mut seen = rungs.iter();
        assert!(
            ladder.iter().filter(|t| t.hears_presence()).all(|t| seen.next() == Some(t)),
            "порядок обязан быть лестничным: {rungs:?} против {ladder:?}"
        );
    }
    ...
    /// Сколько шёл мебибайт по эфиру на связке с телефоном, миллисекунд.
    ///
    /// Число из журнала стенда, а не из головы: чанки класса L приходили
    /// с промежутком 84 секунды в одну сторону, а в другую запись одного
    /// кадра заняла 112. Берётся худшее.
    const MEASURED_BULKY_MS: u64 = 112_000;

    #[test]
    fn the_air_deadline_outlasts_a_measured_mebibyte() {
        // **Разбор одного и того же кадра, посланного дважды.** Скромная
        // скорость эфира стояла двадцать килобайт в секунду — по замеру
        // между двумя машинами. Связка с телефоном вдвое медленнее, и срок
        // ожидания мебибайта выходил короче самой передачи: квитанции
        // не дожидался никто, кадр посылался заново, пока первый ещё шёл
        // по воздуху. В журнале — два одинаковых кадра класса L
        // с промежутком в 67 секунд.
        //
        // Проверка держит не число, а правило: **ждать дольше, чем идёт
        // кадр**. Измерение при этом названо явно, чтобы следующий, кто
        // соберётся править скорость, спорил с замером, а не с числом.
        let waited = receipt_timeout_ms(Transport::Bt, BULKY).expect("у прямой ступени срок есть");
        assert!(
            waited > MEASURED_BULKY_MS,
            "мебибайт идёт {MEASURED_BULKY_MS} мс, а ждём мы {waited}"
        );
    }
    ...
    #[test]
    fn the_retry_schedule_matches_the_spec_and_then_holds() {
        // §10.5: «0:30 — продолжаем стучать; 5:00 — уходим в асинхронные;
        // далее — быстрые раз в 15 минут». Числа повторены здесь, а не
        // взяты из крейта: проверка стережёт обещание расписания,
        // и возьми она константу, растянутый вдвое срок прошёл бы молча.
        assert_eq!(retry_after_ms(0), Some(30_000));
        assert_eq!(retry_after_ms(1), Some(300_000));
        assert_eq!(retry_after_ms(2), Some(900_000));
        // **И у расписания есть конец** (§10.5: «24:00 — прекращаем»).
        // Сумма шагов — сутки: полминуты, пять минут и девяносто пять
        // с лишним четвертей часа.
        let total: u64 = (0..).map_while(retry_after_ms).sum();
        let day = 24 * 60 * 60 * 1000;
        assert!(
            (day - 15 * 60_000..=day + 15 * 60_000).contains(&total),
            "расписание обязано кончаться сутками, а вышло {total} мс"
        );
        assert_eq!(retry_after_ms(97), None, "после суток — «не отвечает», а не новый круг");
    }

    #[test]
    fn a_forgotten_rung_is_offered_again() {
        // Оборванная связь — не отказ ступени: собеседник «обыкновенно
        // звонит снова через секунду». Забытая ступень обязана снова
        // попасть в выбор, иначе один чужой перезапуск сжигает
        // единственный путь.
        let peer = PeerAvailability {
            has_ygg: true,
            enabled: TransportSet::none().with(Transport::Ygg),
            ready: TransportSet::none().with(Transport::Ygg),
            ..PeerAvailability::default()
        };
        let mut attempt = Attempt::new();
        assert_eq!(attempt.next(peer), Some(Decision::Use(Transport::Ygg)));
        // Второй раз ту же ступень лестница не предлагает — она идёт вперёд.
        assert_eq!(attempt.next(peer), Some(Decision::Undeliverable));

        attempt.forget(Transport::Ygg);
        assert_eq!(
            attempt.next(peer),
            Some(Decision::Use(Transport::Ygg)),
            "забытая ступень предлагается заново"
        );
        assert!(!attempt.is_finished(), "и попытка снова жива");
    }
