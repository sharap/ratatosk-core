...
Клиентов два, и оба на Kotlin: телефон (`RatatoskClient`) и второй экран
к нему — компаньон §13.4 (`RatatoskCompanion`). Сборка под телефон —
`ANDROID.md`, под десктоп — `DESKTOP.md`; скрипты `tools/build-android.sh`
и `tools/build-desktop.sh`. Планшет в режиме компаньона собирается как
телефон: платформа та же, отличается только то, что показывает приложение.
