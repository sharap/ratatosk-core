...
fn an_open_channel_works_by_the_link_alone() {
    ...
    // И сказанное **до** подписки — тоже: §7.6 разрешает вытянуть
    // шифротекст всякому, а §10.4 кладёт ключ в ссылку. Это и есть
    // смысл архива §7.2: пришедший позже читает прошлое.
    stand.sleep_for(2 * 60 * 60 * 1000);
    stand.maintenance();
    stand.settle();
    assert!(
        stand.sim.node(NodeId(1)).seen(chat).contains(&"сказано до подписки".to_owned()),
        "архив открытого канала читается тем же ключом; видно {:?}; сид {:#x}",
        stand.sim.node(NodeId(1)).seen(chat),
        stand.sim.seed()
    );
}
