...
impl<S: Store> Engine<S> {
    ...
    pub(super) fn on_handshake_first(
        ...
        // **И привязка к каналу — заново** (§7.5.1). Она живёт ровно
        // столько, сколько живёт связь, и после всякого нового
        // рукопожатия её надо повторить: см. `reattach_to`.
        effects.extend(self.reattach_to(now_ms, peer_ik)?);
    ...
    pub(super) fn on_handshake_response(
        ...
        effects.extend(self.retry_deferred(Some(peer_ik))?);
        // Привязка к каналу — заново, как и на встречной стороне
        // (`reattach_to`): связь новая, значит и «шли мне блоки»
        // сказать надо заново.
        effects.extend(self.reattach_to(now_ms, peer_ik)?);
