...
#
#   1. `mail` не включается никогда: почтового круга у второго экрана нет
#      и не будет — это часы (§5.3), а он про «здесь и сейчас».
#      `tor` — по выбору: без него компаньон ходит только локальной сетью,
#      с ним поднимает свой onion-сервис и достижим из другого города
#      (§13.4, ARCHITECTURE 5вн). Умолчание — без: arti это несколько
#      десятков мегабайт кода, и тому, кто пользуется вторым экраном дома,
#      он не нужен.
#
#      Собрать с ним:  tools/build-desktop.sh --features tor
#      Одного признака мало: клиент обязан ещё передать `torDir` в `open`,
#      иначе сервис не поднимется. Так и задумано — где держать состояние
#      Tor, решает приложение, а не библиотека.
#
#      `bt` — своё радио эфира (`bluer` поверх BlueZ) — включается
#      **сам и только на Linux**: скрипт добавляет признак, увидев Linux,
#      и не добавляет на macOS и Windows. Спрашивать о нём человека
#      незачем: на Linux он нужен всегда (без него у компаньона нет
#      эфира вовсе), а вне Linux BlueZ-а нет.
#
#      Отказаться можно: `--no-bt`. Нужно это одному случаю — сборке
#      на машине без заголовков libdbus, где `bluer` не соберётся вовсе.
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --no-bt) WANT_BT="no"; shift ;;
        *) echo "неизвестный ключ: $1" >&2; exit 2 ;;
    esac
done

# Эфир — только там, где есть BlueZ, то есть на Linux.
#
# **Добавляется к тому, что попросили, а не вместо.** Человек, написавший
# `--features tor`, не должен из-за этого лишиться эфира: списки признаков
# складываются, а не соперничают. Ровно на этом спотыкаются скрипты,
# где умолчание живёт в той же переменной, что и просьба.
if [[ "$WANT_BT" == "yes" && "$(uname -s)" == "Linux" ]]; then
    if [[ -n "$FEATURES" ]]; then
...
# --- 1. Библиотека ---------------------------------------------------------
#
# Без `--features`: onion не поднимается, почты нет вовсе (см. заголовок).
# `--features tor` даёт живой arti; `torDir` в `open` решает, поднимать ли
# сервис на самом деле.
