...
impl Action {
    /// Расходится ли это действие **веером по составу**.
    ///
    /// # Зачем этот вопрос отдельно от права
    ///
    /// Право говорит, **кому можно**; это — **куда поедет**. В канале
    /// они расходятся: §3.2 оставляет состав владельцу, поэтому веером
    /// в звезде возит только он ([§7.5.2]), а вот адресованный блок —
    /// ключ чтения впущенному, запись о впуске владельцу — делегат
    /// отправляет как прежде, и ничьего состава для этого не нужно.
    ///
    /// Спутать их легко и дорого: запрети мы делегату всё по праву, он
    /// перестал бы впускать; разреши всё — слово уехало бы в пустоту,
    /// и человек увидел бы «отправлено» там, где не отправлено никому.
    ///
    /// # Перебор исчерпывающий, без `_`
    ///
    /// Новый вид действия обязан **ронять сборку**, а не оказываться
    /// веерным молча: цена ошибки здесь — либо тишина вместо доставки,
    /// либо отказ там, где отказывать не за что.
    ///
    /// [§7.5.2]: рататоск, фаза 2, §7.5.2
    #[must_use]
    pub const fn fans_out(&self) -> bool {
        match self {
            // Всё, что человек кладёт в чат: копию получает каждый.
            Action::Edit { .. }
            | Action::Retract { .. }
            | Action::Reaction { .. }
            | Action::Reply { .. }
            ...
            | Action::Rename { .. }
            | Action::Avatar { .. } => true,
            // Блок ключа чтения назван адресатом внутри (§5.3): он едет
            // одному, и ретранслировать его вправе кто угодно, не читая.
            Action::ArchiveKey { .. } => false,
            // Запись о впуске — учёт, и смотрит его владелец (§6.5).
            // Едет ему одному: разослав её читателям, мы рассказали бы
            // каждому, кто ещё читает канал.
            Action::Admission { .. } => false,
            // Представление отдаётся впущенному вместе со впуском —
