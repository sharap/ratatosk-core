...
/// Номер шага у кадра отзыва (§13.4).
///
/// Двойка, потому что шагов у рукопожатия два — нулевой и первый, — и третьего
/// не бывает. Кадр с этим номером рукопожатием не является: он говорит, что
/// рукопожатия не будет. Телефон его только отправляет, десктоп только
/// принимает; обратного направления у него нет и быть не может.
pub const REVOKED_STEP: u64 = 2;
...
/// Собирает кадр отзыва — ответ рукопожатию, которого не будет (§13.4).
///
/// **Кадр рукопожатия по типу и запечатанный нашим AEAD по содержимому**,
/// и это единственное такое место. Оба выбора вынужденные, и оба стоит
/// назвать вслух.
///
/// Тип — рукопожатие, потому что кадр и есть ответ на рукопожатие: отозванный
/// десктоп постучался, и ему отвечают, что стучаться больше некуда. Заводить
/// ради этого третий тип кадра значило бы менять §7 — общий провод всех
/// разговоров — ради одного случая в разговоре с собственным десктопом.
/// Различает его номер шага: [`REVOKED_STEP`], которого в рукопожатии нет.
///
/// Запечатан нашим AEAD, потому что Noise здесь не было и не будет. У обычного
/// кадра рукопожатия содержимое уже зашифровал Noise, и второй раз его
/// не шифруют; здесь шифровать некому — сессии между этими двумя больше
/// не существует. Ключ выводится статическим DH
/// (`ratatosk_crypto::companion::revocation_key`), и это единственный
/// известный обоим ключ, для которого не нужно ни одного кадра.
///
/// Нагрузка **пустая**, и это не забывчивость: сказать надо одно, и это одно
/// в самом факте кадра, который открылся. Любое поле здесь пришлось бы
/// объяснять человеку, а объяснять нечего.
///
/// # Errors
///
/// Отказ запечатывания.
pub fn revoked(
    ...
    let header =
        Header::new(FrameType::Handshake, ratatosk_wire::HANDSHAKE_SESSION_ID, REVOKED_STEP, nonce);
    aead::seal(key, &header, HANDSHAKE_CLASS, &[])
}

/// Проверяет кадр отзыва: открылся — значит его собрал наш телефон.
///
/// Открывать нечего — нагрузка пуста, — и весь ответ в том, сошёлся ли тег.
/// Подделать его может лишь владелец одного из двух статических секретов;
/// повторить перехваченный — только тому же десктопу, который и так отозван.
///
/// # Errors
///
/// Кадр не наш, испорчен или собран не тем ключом.
pub fn open_revoked(key: &[u8; 32], frame: &[u8]) -> Result<(), CryptoError> {
    aead::open(key, frame).map(|_| ())
}
...
pub fn open(session: &mut Session, frame: &[u8], now_ms: u64) -> Result<Vec<u8>, CryptoError> {
    let view = ratatosk_wire::parse(frame)?;
