...
pub struct GroupStatus {
    ...
    ///
    /// **Отдаётся затем, чтобы отказ не понадобился.** Предел закрытой
    /// группы — тридцать два, и узнавать о нём из неудачной попытки
    /// человеку незачем: с этим числом клиент гасит «добавить» заранее
    /// и говорит, сколько мест осталось.
    ///
    /// Считает ядро, а не клиент вычитанием длины состава из предела:
    /// это ровно то знание о протоколе, которое §13.3 держит ниже границы,
    /// и считается оно там же, где стоит само правило приглашения —
    /// [`ratatosk_proto::Group::free_slots`] зеркалит `invite`.
    ///
...
impl<S: Store, R: Runner> Driver<S, R> {
            ...
            Query::Groups { reply } => {
                ...
                let found = self
                    ...
                    .map(|(chat, state)| GroupStatus {
                        ...
                        // Вышедший никого не зовёт — значит и мест у него нет.
                        free_slots: if state.group.contains(&me) {
                            state.group.free_slots()
                        } else {
                            0
                        },
    ...
    /// Куда звонить этому ключу — спрашивается у ядра.
    ///
    /// **Одно место на драйвер и на стенд.** Правило «контакт, иначе пир,
    /// иначе устройство» живёт в `Engine::address_of`: разойдись копии,
    /// стенд зеленел бы там, где живой раннер отвечает `NoAddress`, —
    /// а именно так и выглядела бы тишина, которую нечем объяснить.
    fn address_of(&self, peer_ik: [u8; 32]) -> PeerAddress {
        self.engine.address_of(&peer_ik)
    }
}
