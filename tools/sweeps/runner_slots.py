...
ALLOWED = {
    ...
    # Шестая ступень (Bluetooth, 0.4). Строка стенда отсюда уже ушла:
    # там стоит настоящий раннер под признаком `bt`. Остались два случая,
    # и оба честные — телефон и терминал компаньона.
    ("crates/ffi/src/lib.rs", "Bt"): "раннер на `bluer` — только Linux; Android ждёт своей (0.5)",
    ("crates/lab/src/main.rs", "Bt"): "терминал компаньона говорит с телефоном, а у того эфира нет",
    ("crates/ffi/src/companion.rs", "Bt"): "и у второго экрана его тем более нет",
}
