...
/// Пир своего узла — как его видит библиотека прямо сейчас (0.2).
///
/// Живое состояние, а не настройка: настройку человек назвал сам, и она
/// лежит в [`YggSetup::Embedded`]. Здесь — что из неё работает.
///
/// **Названный, но мёртвый пир из списка не исчезает** — он остаётся
/// с `up = false`, сохранённым `uri` и нулями во всём остальном. Это
/// наблюдение с живого узла, а не обещание библиотеки, но оно избавляет
/// от сшивки с настроенным списком: живой состав и так называет всех.
/// Разойдись однажды одно с другим — увидим пира, которого нет в настройках,
/// и это будет видно глазами.
///
/// # Зачем это человеку
///
/// Пир — чужой узел, который кто-то держит по доброй воле. Он может
/// исчезнуть навсегда: владелец выключил ноду, кончился хостинг, сменился
/// адрес. Без живого состава «меш не работает» и «один из трёх пиров умер
/// полгода назад» выглядят одинаково, а чинятся по-разному.
///
/// # Почему полей четыре, а в источнике их вдвое больше
///
/// Библиотека отдаёт ещё счётчики байтов, стоимость, приоритет, время
/// жизни и ключ узла. Ни одно из них не отвечает на вопрос, который человек
/// задаёт экрану настроек, — «какую строку отсюда убрать», — а каждое
/// пришлось бы нести через границу UniFFI и показывать. Понадобятся —
/// добавятся по одному, с объяснением, зачем.
...
pub struct YggPeer {
    ...
    pub uri: String,
    /// Соединение с этим пиром работает.
    pub up: bool,
    /// Соединение начал **он**, а не мы.
    ///
    /// Такого пира в настройках нет и быть не должно: убрать его нельзя,
    /// потому что мы его не добавляли. Показывать его отдельно от своих
    /// обязательно — иначе человек будет искать в списке строку, которой
    /// там нет.
    pub inbound: bool,
    /// Задержка до пира, мс. Ноль у неживого — он и не измерялся.
    ///
    /// Не для §5.4: лестница на это число не смотрит. Для человека,
    /// выбирающего между двумя работающими пирами.
    pub latency_ms: f64,
}
