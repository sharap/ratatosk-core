...
/// Сколько живёт привязка читателя без повторной (§7.5.1).
///
/// Читатель называется снова на каждом обходе — раз в час — и на каждой
/// новой сессии; три часа тишины — это три пропущенных обхода, а не
/// один медленный. Отказы ступеней признаком ухода не служат: они
/// штатны и у живого.
pub const ATTACH_TTL_MS: u64 = 3 * 60 * 60 * 1000;
...
/// Сколько блоков кладётся в один пакет асинхронной ступени (§8.4).
///
/// §8.4: «Асинхронные ступени не бывают lazy… Им шлётся пакет блоков
/// за окно с избыточностью». Избыточность здесь — не украшение:
/// на почте и реле ответа не обещают (§5.3, 0.3), `GRAFT` там
/// не зовётся вовсе ([`graft_wait_ms`]), и потерянный блок ждал бы
/// анти-энтропии — то есть следующего обхода, то есть часа.
///
/// Три — это «письмо вместо трёх писем» и втрое больший шанс, что
/// пропущенное приедет само. Больше — и цена пакета начинает спорить
/// с ценой ожидания: блоки в канале невелики, но письмо с двумя
/// десятками уже не мелочь.
pub const BUNDLE_BLOCKS: usize = 3;

/// Сколько байт пакет не перерастает (§8.4, §9.3).
///
/// Свыше этого блоки едут по одному: письмо, которое приходится резать
/// на части (§9.3), теряет ровно то, ради чего пакет и собран, —
/// один проход вместо трёх.
pub const BUNDLE_BYTES: usize = 64 * 1024;

/// Пакет блоков для асинхронной ступени (§8.4).
#[must_use]
pub fn bundle_value(blocks: &[Vec<u8>]) -> Value {
    Value::Array(blocks.iter().map(|block| Value::Bytes(block.clone())).collect())
}

/// Разбирает пакет блоков.
///
/// # Errors
///
/// [`ChannelError::Malformed`] — не массив байтовых строк либо блоков
/// больше предела: пакет, пришедший из будущей сборки, разбирается
/// целиком или никак.
pub fn bundle_from_value(value: &Value) -> Result<Vec<Vec<u8>>, ChannelError> {
    let Value::Array(items) = value else { return Err(ChannelError::Malformed) };
    if items.len() > BUNDLE_BLOCKS {
        return Err(ChannelError::Malformed);
    }
    let mut blocks = Vec::with_capacity(items.len());
    for item in items {
        let Value::Bytes(bytes) = item else { return Err(ChannelError::Malformed) };
        blocks.push(bytes.clone());
    }
    Ok(blocks)
}

/// Сколько блоков тянет **одна прокрутка вверх** (§7.4, шаг 3).
///
/// «Архив — по требованию, при прокрутке вверх»: человек долистал
/// до начала того, что у него есть, и попросил ещё. Страница, а не
/// вся лента: спросивший год переписки заплатил бы за него сразу
/// и целиком, а прочитает он экран.
///
/// Число своё, не из §7.2: предел ответа (128 блоков) — про то,
/// сколько влезает в один ответ, а это про то, сколько человек просит
/// одним движением.
pub const HISTORY_PAGE: u64 = 32;
...
/// Сколько записей держим в каталоге одного канала (§7.5).
///
/// # Предел нужен **открытому** каналу, и только ему
///
/// У канала по приглашению каталог ограничен составом: вписаться
/// в него может лишь тот, кого владелец впустил. У открытого состава
/// нет вовсе (§10.4), а читать его §7.6 разрешает всякому, у кого есть
/// идентификатор, — значит и назваться раздающим может всякий, и без
/// предела каталог рос бы, пока хватает диска.
///
/// # Почему «полный каталог не берёт нового», а не «вытесняет давнего»
///
/// Вытеснение выглядит честнее, но им же и пользуются: заваливший
/// каталог свежими записями выбил бы оттуда настоящих раздающих одного
/// за другим. Отказ новому оставляет флудеру только первый ход — занять
/// места до того, как их займут другие, — и §7.7 отвечает на это
/// остыванием: место в списке ничего не стоит, если по нему не отвечают.
///
/// Число взято от `MAX_ATTACHED_SEEDS` (ядро, §7.1) с большим запасом:
/// привязка держит 3–5 пиров, и каталог на три десятка даёт из чего
/// выбирать даже когда половина записей протухла. Ссылкой это не
/// связано нарочно — крейт протокола о ядре не знает и знать не должен.
...
///
/// Первая редакция брала срок ответа ступени — «чтобы числа
/// не разошлись». Вопрос у них, однако, разный. Срок ступени отвечает
/// «когда считать собеседника недостижимым» и потому велик: у onion это
/// полторы минуты. Здесь же спрашивается другое — «сколько ждать блок,
/// который, может быть, уже едет от другого». Блок едет секунды,
/// и ждать его полторы минуты значит ровно на столько и задержать
/// ленту.
///
/// Живой прогон это и показал: читатель, оказавшийся ленивым **и
/// у владельца, и у сида** (так бывает — дерево сходится подрезкой),
/// получал слово через полторы минуты после всех. Снаружи это
/// «сообщения приходят с задержкой, а иногда будто и не приходят».
///
/// Числа поэтому свои, и они ровно те, что называет §7.7: «секунды
/// в LAN, десятки секунд в onion».
///
...
pub fn graft_wait_ms(via: crate::transport_policy::Transport) -> Option<u64> {
    ...
    match via {
        // В локальной сети блок едет миллисекунды: две секунды — это уже
        // «он не придёт».
        Transport::Lan => Some(2_000),
        // Эфир медленнее провода на порядок, и класс S там мал (§8.4).
        Transport::Bt => Some(5_000),
        // Меш — сеть поверх сети: десяток хопов и переустановка пути.
        Transport::Ygg => Some(8_000),
        // Onion: три узла в каждую сторону, и §7.7 называет для него
        // «десятки секунд».
        Transport::Onion => Some(20_000),
