...
impl<S: Store> Engine<S> {
    ...
    pub(super) fn note_heard(
        ...
        // Собеседник нашёлся — расписание повторов §10.5 начинается
        // заново: ступенька в четверть часа осмысленна для того,
        // кто молчит сутки, и бессмысленна для того, кто только что
        // объявился.
        self.retry_step.remove(&peer_ik);
