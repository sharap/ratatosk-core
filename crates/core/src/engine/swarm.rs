...
impl<S: Store> Engine<S> {
    ...
    pub(super) fn may_serve(&self, chat: ChatId, peer_ik: &[u8; 32]) -> Result<bool, EngineError> {
        // Принимать при этом можно всегда: §12 начинается с того, что
        // принимать почти бесплатно, а платит — отдающий.
        // **Свой канал отдаётся всегда, и это выше правила §12.**
        // Владелец — источник, а не сид: §7.5.2 обещает, что «ноль сидов
        // — это звезда, и она обязана работать как состояние». Запрети
        // мы ему отдавать, канал без сидов перестал бы существовать.
        //
        // Проверка стояла **под** признаком экрана, и та его гасила:
        // аккаунт, ушедший с экрана, переставал принимать привязки,
        // отвечать на просьбы и чинить дерево — в собственном канале.
        // Снаружи это «канал работает, только пока автор смотрит
        // в экран», и лечилось оно тем, что кто-то объявлял себя сидом.
        //
        // §12 говорит о **втором аккаунте** на том же устройстве
        // («активен ровно один… его сидирование прекращается»), а не
        // о свёрнутом окне. Раздача чужого под ним и остаётся.
        let me = self.identity.public().ik;
        if self.channel_owner(chat).is_some_and(|owner| owner == me) {
            return Ok(true);
        }
        // **Фоновый аккаунт не раздаёт чужое** (§12). Второй аккаунт,
...
impl<S: Store> Engine<S> {
    ...
    pub(super) fn on_pull_older_history(
        ...
        // **А у канала по приглашению спрашивать надо владельца**, и он
        // не цель привязки (§7.5.2: он развозит по составу, и «шли мне
        // блоки» от читателя развалило бы дерево). Без этой строки
        // прокрутка вверх там не спрашивала **никого**: целей ноль,
        // и человек получал «дальше некуда» на канале, у которого
        // история есть.
        //
        // Едет не привязка, а вектор: «вот что у меня есть». Ответный
        // вектор владельца и покажет, чего не хватает, — а граница
        // к этому мигу уже опущена на страницу (`history_pull`).
        let me = self.identity.public().ik;
        if effects.is_empty() {
            if let Some(owner) = self.channel_owner(chat).filter(|owner| *owner != me) {
                match self.send_have(now_ms, chat, owner) {
