...
impl<S: Store> Engine<S> {
    ...
    /// поэтому и раздаёт эту команду всем (`multi::send_to_all`).
    ///
    ...
    pub(super) fn watch_peers(&self) -> Effect {
        ...
        //
        // **Без повторов**, и это не косметика. Владелец канала лежит
        // пиром, а после первого же рукопожатия становится ещё
        // и несверенным контактом (приёмной стороны §8.3 пока нет) —
        // и попадал в список дважды. Эфир на это отвечал «известных=2
        // ключи=afabc241,afabc241»: счёт наблюдаемых врал, а каждое
        // чужое объявление сверялось с одним и тем же ключом по два раза.
        // Найдено в логе живого стенда.
        Effect::WatchPeers(
            self.contacts
                .keys()
    ...
    pub(super) fn on_link_down(
        ...
        // **Забытый собеседник — не отказ ядра.** Отписка (§10.6)
        // и удаление контакта убирают запись, а кадры к нему могут быть
        // ещё в пути: транспорт доложит об обрыве позже, чем мы забыли.
        // Двигать по лестнице §5.4 тогда нечего — адреса нет, — а падать
        // на этом нельзя: отказ вышел бы из `step` и уронил **весь** шаг,
        // вместе со всем, что в нём ехало к другим.
        //
        // Нашёл это стенд на возвращении в канал: человек отписался,
        // запись владельца ушла вместе с подпиской, а блок ухода ещё
        // стучался.
        if !self.contacts.contains_key(&peer_ik) && !self.peers.contains_key(&peer_ik) {
            tracing::debug!(peer = ?&peer_ik[..4], "обрыв к забытому собеседнику — двигать нечего");
            return Ok(Vec::new());
        }
