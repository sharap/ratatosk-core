...
impl<S: Store> Engine<S> {
    ...
    fn source_for(&mut self, file: &StoredFile) -> Result<Option<[u8; 32]>, EngineError> {
        ...
        // **Отправитель — запасной путь, а не первый.**
        //
        // §9.1 начинается с этого: «отправитель выгружает файл **один
        // раз**, дальше куски расходятся между участниками. Было бы
        // иначе — мебибайт при 32 участниках это 32 мебибайта
        // с отправителя».
        //
        // Пока он стоял здесь первым, так и было: при живом отправителе
        // все шли к нему, и рой вложений не работал ни разу — держатели
        // объявляли карту, которую никто не спрашивал. Теперь у него
        // берут, только когда взять больше не у кого.
        //
        ...
        // так куски и чередуются между источниками. Один выбор на файл
        // означал бы, что весь мебибайт едет от одного — то есть ровно
        // то, чего §9.1 не хочет.
        let mut raw = [0u8; 4];
