//! Граница целиком, ровно так, как её увидит Kotlin.
//!
...
#[test]
fn the_air_says_it_has_a_radio_exactly_when_it_has_one() {
    // **Вопрос у `has_radio` — «есть ли чем поднимать эфир», а не «вручали
    // ли нам».** Разница видна ровно в одной сборке — Linux с признаком
    // `bt`, где радио своё (`bluer`), — и там она решающая: буквальный
    // ответ «нам ничего не вручали» заставил бы клиента написать «эфир
    // недоступен» над живой ступенью.
    //
    // Проверка идёт в обе стороны признака, потому что ветки разные
    // и разойтись им нельзя: без своего радио ответ обязан остаться
    // отрицательным, иначе §5.4 сочтёт ступень годной и станет жечь
    // на ней попытки.
    let db = TempDb::new("bt-radio");
    let client = RatatoskClient::open(db.path(), Some("1234".to_owned()), None, "я".to_owned())
        .expect("клиент открылся");
    let air = client.bluetooth();

    if cfg!(all(feature = "bt", target_os = "linux")) {
        assert!(air.has_radio(), "у сборки со своим радио эфир обязан числиться поднимаемым");
    } else {
        assert!(
            !air.has_radio(),
            "радио платформы не вручали — говорить обратное значит обещать ступень, которой нет"
        );
    }
