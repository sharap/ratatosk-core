...
/// Текст для UI, когда файлу не на чем ехать (§10.3).
///
/// Раньше назывался «ждёт прямого канала» и говорил правду: почтой чанки
/// не ходили вовсе. Теперь ходят, и состояние стало реже и уже — канала
/// нет **никакого**, либо файл слишком велик для почты ([`may_send_over`])
/// и ждёт прямого. Текст от этого не меняется: человеку важно, что файл
/// не потерян и поедет сам.
#[must_use]
pub const fn waiting_for_channel_text() -> &'static str {
...
mod tests {
    ...
    fn the_stall_deadline_outlasts_a_window_on_every_transport() {
        ...
        for via in [Transport::Onion, Transport::Mail] {
            ...
            let needed_ms = window_bytes * hops * 1_000 / SLOW_BYTES_PER_SEC;
            assert!(
